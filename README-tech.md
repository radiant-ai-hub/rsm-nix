...
and `skills/`. There are two deploy paths, on purpose:

**1. Skills — user-level (`~/.claude/skills/`).** Installed by
`rsm-setup` (§6b), which copies every `skills/<name>/` from the flake.
Available in every folder, including personal work (they are
guidance-only, so this is harmless):
...
- `verify-ai-code` — a structured read-and-verify pass on AI-written
  code (read the diff, explain it, run + manually exercise, check the
  agent’s claims).

Adding a skill is just a new `skills/<name>/SKILL.md` — no code change.

**2. Per-folder harness (`<dir>/.claude/` + `CLAUDE.md` + `justfile`).**
Written by `rsm-claude-settings <dir>`, which `rsm-setup` calls on the
`~/rsm-msba` workspace root and `rsm-mkdir`/`rsm-clone` call on every
folder — mirroring `rsm-vscode-settings`. Curated source is read from
the **live** flake checkout (`$RSM_FLAKE/claude/`), so a `git pull` of
the flake updates it — no rebuild. The harness is therefore active
**only** in folders the environment set up; a student’s personal
projects elsewhere are untouched.
...
- `.claude/settings.json` — a **uv-only permission policy** (`deny`
  ...
  clobbered (same for a folder’s own `CLAUDE.md`/`justfile`, which are
...
- `.claude/commands/` — `/review`, `/explain`, `/run-tests`,
  `/add-function`.
...
**Toggling hooks / status line (teaching).** The justfile ships
`just hooks-off` / `just hooks-on` (flip the built-in `disableAllHooks`
in `.claude/settings.local.json`, to demo what the hooks catch) and
`just status-line` / `just status-line-off` (install
`claude/statusline.sh` into `~/.claude/settings.json` — shows
`model | context left | 5h & 7d usage`, the rate-limit fields being
Claude Pro/Max only).
...
Verified by `tests/check-claude-harness.sh` (in `nix-ci.yml`): deploy,
secret-scan, the telemetry scope guard (personal / no-origin repos are
never logged), stale-tests, ruff-format, justfile recipes, and the
keep-foreign guards.
