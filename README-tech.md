...
### Onboarding students — single tailnet vs. node sharing

- **One org tailnet + SSO + ACLs (recommended if budget allows).**
  Students sign in with UCSD identity, join the tailnet, and an ACL
  limits them to the server’s SSH/HTTPS ports only (tag the server
  `tag:msba-server`, put students in `group:students`). Centrally
  managed; students just run `tailscale up` and log in. Note the free
  tier is limited on **users** (not just devices), so a cohort needs a
  paid plan.
- **Node sharing.** Share the server node into each student’s own (free)
  tailnet. Avoids the per‑user cost but is per‑student tedium and pushes
  tailnet management onto each student.
