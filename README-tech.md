...
- **Dedicated MSBA tailnet + SSO + ACLs (recommended at class scale).**
  Create a *separate* tailnet for the course (not your personal one).
  Students sign in with UCSD identity; an ACL limits them to the
  server’s SSH/HTTPS ports and blocks student-to-student traffic — tag
  the server `tag:msba-server`, put students in `group:students`, and
  allow only `group:students -> tag:msba-server:[22,443]`. Students just
  run `tailscale up` and log in. The free tier is limited on **users**
  (not just devices), so a cohort needs a paid plan.
- **Node sharing (simplest for a few users).** “Sharing” a node gives
  one student access to *only* the server. The student stays in **their
  own** tailnet — they never join yours and never see your other
  machines — and because each student is in a separate tailnet, students
  can’t see each other either. Trade-off: it is one share **per
  student**, so it is tedious for a large cohort.
