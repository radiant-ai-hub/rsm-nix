...
Every student’s `rsm-pg-start` would bind `127.0.0.1:8765` — a collision
on one host. The `rsm-pg-*` scripts honor `$PGPORT`, so give each UID
its own:
...
collides; this covers the TCP listener and `pgweb`.)
